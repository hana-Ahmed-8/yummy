...
$(document).ready(() => {
    ...
    $('.nav-content, .side-menu').css('left', -navContentWidth);

    $('.menu-controller-icon span').click(function () {
        navContentWidth = $('.nav-content').innerWidth(); // recalculate
        $(this).toggleClass('fa-bars fa-xmark'); // swap icon

        if ($('.side-menu').css('left') == '0px') {
            $('.nav-content, .side-menu').animate({ left: -navContentWidth }, 500);
            $('nav li').animate({ top: 300 }, 500);
            
        } else {
            $('.nav-content, .side-menu').animate({ left: 0 }, 500);
            for (let i = 0; i < 5; i++)
    ...
    $('nav li a').click(function () {
        navContentWidth = $('.nav-content').innerWidth();
        $('.nav-content, .side-menu').animate({ left: -navContentWidth }, 500);
        $('.menu-controller-icon span').removeClass('fa-xmark').addClass('fa-bars');
    });
});
getMealsByName();
